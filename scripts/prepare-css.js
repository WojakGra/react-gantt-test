...
const rollupCssFilePath = path.join(
  process.cwd(),
  './package/dist/esm/index.css',
);

if (!fs.existsSync(rollupCssFilePath)) {
  console.error('CSS file not found. Please run `npm run build` first.');
  process.exit(1);
}
...
fs.writeFileSync(
  path.join(process.cwd(), './package/dist/styles.css'),
  content,
);
fs.writeFileSync(
...
console.log('✓ Prepared CSS files');
